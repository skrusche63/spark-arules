...
class TopKActor(req:ServiceRequest) extends Actor with SparkActor {
  ...
  def receive = {
    ...
    /*
     * Retrieve Top-K association rules from an appropriate 
  ...
  private def findRules(dataset:RDD[(Int,Array[Int])],k:Int,minconf:Double) {
